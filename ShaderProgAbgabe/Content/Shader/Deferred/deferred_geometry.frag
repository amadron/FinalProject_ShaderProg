...
void main()
{
	...
	vec2 uv = inData.uv * 1 - sphereProj;
	float alpha = getAlpha(hasAlphaMap, alphaSampler, uv);
	if(alpha < 0.1f)
		discard;
	...
	vec3 normalSum = inNormal * (1 - hasNormalMap) +  -(texture(normalSampler, uv).rgb * 2f - 1f) * hasNormalMap;
	...
	vec4 color = vec4(materials[inData.material],1) * (1 - hasAlbedo) +	texture(albedoSampler, uv) * hasAlbedo;
